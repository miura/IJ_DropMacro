...
public class Drop_Macro extends PlugInFrame implements DropTargetListener, Runnable, ActionListener {
	...
	public void phantomMacroGenerator(File f){
		f.mkdir();
	...
	public Drop_Macro() {
		...
		l.setText("Drag a macro here");
		File f = new File(dropletActionsPath);
		if (!f.exists()) {
			f.mkdir();
			String exampleMacro=" file=getArgument();\\n open(file);\\n run (\"8-bit\");";
			IJ.runMacro("f=File.open(getDirectory('plugins')+'Droplet Actions'+File.separator+"+"'Open as 8-bit.ijm'"+"); print(f,'"+exampleMacro+"');File.close(f);");
		}
		String[] list = f.list();
		if (list.length==0) { IJ.error(f.toString()+" is empty"); return;}
	...
	public void drop(DropTargetDropEvent dtde)  {
		...
		try  {
			...
			if (iterator!=null) {
				Thread thread = new Thread(this, "Droplet_");
	...
	public void run() {
		...
		while(iterator.hasNext()) {
			...
			try {
				...
				IJ.runMacroFile(dropletActionsPath+c.getSelectedItem()+exts.getItem(c.getSelectedIndex()),path);;
			} catch (Throwable e) {
